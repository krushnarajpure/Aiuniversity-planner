...
export const copilotResponseSchema = z.discriminatedUnion("type", [
    ...
    sections: z
      .array(
        z.object({
          ...
          bullets: z.array(z.string()).optional(),
          kind: z
            .enum(["normal", "example", "warning", "recommendation"])
            .default("normal"),
        }),
      )
      .optional(),
  ...
  responseBase.extend({
    ...
    message: z.string(),
    summary: z.object({
      totalStudyHours: z.number().nonnegative(),
      subjects: z.array(z.string()),
      highPriorityTopics: z.array(z.string()),
      revisionSessions: z.number().int().nonnegative(),
      mockTests: z.number().int().nonnegative(),
    }),
    sessions: z.array(
      z.object({
        time: z.string(),
  ...
  responseBase.extend({
    type: z.literal("weekly_timetable"),
    message: z.string(),
    sessions: z.array(
      z.object({
        day: z.string(),
...
export async function getCopilotContext(
  ...
  return {
    ...
    assignments: assignments.map((assignment) => ({
      ...
      deadline: toDate(assignment.deadline),
    ...
    studyMaterials: studyMaterials.map((material) => ({
      ...
      unit: material.unit,
      notes: material.notesContent?.slice(0, 10000) ?? null,
      tags: material.tags,
    })),
...
export function buildCopilotPrompt(
  ...
  const modeInstruction = {
    "study-coach":
      "Be practical and encouraging. Turn priorities into a realistic schedule.",
    tutor:
      "Teach clearly from first principles, with examples and a short revision recap.",
    "academic-analyst":
      "Analyze the supplied academic data, call out patterns and uncertainty, and recommend priorities.",
    "exam-coach":
      "Focus on exam date, high-yield topics, practice, and spaced revision.",
    "career-mentor":
      "Connect academic work to skills and career direction without inventing personal facts.",
    "doubt-solver":
      "Resolve the student's question step by step and check likely misconceptions.",
    "productivity-coach":
