...
export async function getOrganizationApplication(applicationId: string) {
    const user = await requireOrganization();
    return prisma.placementApplication.findFirst({
        where: { id: applicationId, job: { organizationId: user.organization!.id } },
        include: {
            student: { include: { placementProfile: true } },
