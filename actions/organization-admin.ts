...
import { sendOrganizationApprovedEmail, sendOrganizationRejectedEmail } from "@/lib/email";
...
export async function updateOrganizationVerification(organizationId: string, status: "PENDING" | "APPROVED" | "REJECTED" | "SUSPENDED", message?: string) {
    ...
    const updated = await prisma.organization.update({ 
      ...
      data: { 
        ...
        verificationMessage: message?.trim() || (status === "APPROVED" ? "Organization approved by administrator." : status === "REJECTED" ? "Organization registration requires changes. Please contact support." : status === "SUSPENDED" ? "Organization account has been suspended." : "Registration is under review.") 
      } 
    });
