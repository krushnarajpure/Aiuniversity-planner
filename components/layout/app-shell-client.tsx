...
export function AppShellClient({
  ...
  useEffect(() => {
    ...
    fetch("/api/notifications/unread-count", { cache: "no-store", signal: controller.signal })
      .then(async (response) => response.ok ? response.json() as Promise<{ unreadCount: number }> : null)
      .then((data) => {
        if (data && Number.isFinite(data.unreadCount)) setCurrentUnreadCount(data.unreadCount);
      })
      .catch(() => undefined);
  ...
  return (
    <div className="flex min-h-screen bg-background-light dark:bg-background-dark">
      ...
      <div className="flex-1 min-w-0">
        <Navbar userName={userName} unreadCount={currentUnreadCount} onMenuClick={() => setMobileOpen(true)} />
        <main>
