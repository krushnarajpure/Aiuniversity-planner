...
const metricIcons: {
  label: string;
  key: "all" | "pending" | "approved" | "rejected" | "suspended";
  icon: LucideIcon;
}[] = [
...
function meetingDuration(
  meeting: Organization["user"]["hostedMeetings"][number],
) {
  if (!meeting.startedAt) return 0;
  return Math.max(
    0,
    Math.round(
      (new Date(meeting.endedAt ?? new Date()).getTime() -
...
export function OrganizationApprovals({
  ...
  const visible = organizations.filter(
    (item) =>
      (filter === "ALL" ||
        ...
        (filter === "APPROVED" && item.verificationStatus === "VERIFIED")) &&
      `${item.companyName} ${item.user.email} ${item.recruiterName ?? ""} ${item.industry ?? ""}`
        .toLowerCase()
  ...
  async function changeStatus(
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          payload.error || "Unable to update organization status.",
        );
                ...
                verificationMessage:
                  ...
                  (nextStatus === "APPROVED"
                    ...
                    : nextStatus === "SUSPENDED"
                      ? "Organization suspended by administrator."
      ...
      setRejecting(null);
      setReason("");
      toast.success(`Organization ${nextStatus.toLowerCase()}.`);
    } catch (error) {
      toast.error(
  ...
  async function deleteOrganizationById(id: string) {
    const confirmed = window.confirm(
      "Delete this organization and its jobs permanently?",
    );
    if (!confirmed) return;
    setBusy(id);
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {metricIcons.map(({ label, key, icon: Icon }) => (
          <div
            key={label}
          ...
          >
            ...
            <p className="mt-3 text-2xl font-semibold">{counts[key]}</p>
          </div>
        ))}
      </div>
      <div className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex gap-1 overflow-x-auto">
          {statuses.map((item) => (
            <button
              ...
              className={`whitespace-nowrap rounded-lg px-3 py-2 text-xs font-medium ${filter === item ? "bg-primary text-white" : "text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800"}`}
            >
              {item}
            </button>
          ))}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="rounded-lg border border-slate-200 p-2 text-slate-500 hover:bg-slate-100 dark:border-slate-700 dark:hover:bg-slate-800 disabled:opacity-50"
          >
            <RefreshCw
        ...
        <div className="space-y-4">
          {visible.map((organization) => (
            ...
            >
              <div className="flex flex-col gap-4 lg:flex-row lg:justify-between">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusStyle(organization.verificationStatus)}`}
                    >
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    ...
                    {organization.location || "Location not provided"}
                  </p>
                  <p className="mt-3 text-sm">
                    {organization.recruiterName || organization.user.name} ·{" "}
                    {organization.recruiterDesignation || "Recruiter"}
                  </p>
                  <p className="mt-1 break-all text-xs text-slate-500">
                    {organization.user.email} ·{" "}
                    {organization.user.emailVerified
                      ? "Email verified"
                      : "Email not verified"}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    onClick={() => changeStatus(organization.id, "APPROVED")}
                  ...
                  {organization.verificationStatus === "APPROVED" && (
                    <button
                      ...
                      onClick={() => changeStatus(organization.id, "SUSPENDED")}
              ...
              <div className="mt-4 flex flex-col gap-2 rounded-lg bg-slate-50 p-3 text-xs text-slate-600 dark:bg-slate-800 dark:text-slate-300 sm:flex-row sm:justify-between">
                <span>
                  {organization.verificationMessage ||
                    "Awaiting administrator review."}
                </span>
                <span>
                  {organization._count.jobs} jobs · Registered{" "}
                  {formatDate(organization.user.createdAt)}
                </span>
              </div>
              <div className="mt-3 rounded-lg border border-slate-200 p-3 dark:border-slate-700">
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Hosted meetings ({organization.user.hostedMeetings.length})</p>
                {organization.user.hostedMeetings.length ? <div className="mt-2 space-y-2">{organization.user.hostedMeetings.map((meeting) => <div key={meeting.meetingCode} className="flex flex-col gap-1 text-xs sm:flex-row sm:items-center sm:justify-between"><span className="font-medium">{meeting.title} · {meeting.status}</span><span className="text-slate-500">{meetingDuration(meeting)} min · {meeting.participants.length} users</span></div>)}</div> : <p className="mt-2 text-xs text-slate-500">No meetings hosted yet.</p>}
              </div>
            </article>
          ))}
        </div>
      ...
      {rejecting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4">
          ...
          >
            ...
            <p className="mt-1 text-sm text-slate-500">
              Add a reason for the recruiter.
            </p>
            <textarea
              autoFocus
              value={reason}
              ...
              className="mt-4 min-h-28 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 text-sm dark:border-slate-600"
            />
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button
                ...
                onClick={() =>
                  changeStatus(rejecting.id, "REJECTED", reason.trim())
                }
