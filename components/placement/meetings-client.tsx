...
export function MeetingsClient({ userName, initialMeetingId = "" }: { userName: string; initialMeetingId?: string }) {
  ...
  async function createMeeting() {
    ...
    try {
      ...
      setMeetingId(result.meeting.id); setPassword(result.meeting.password); setMode("join");
      await joinMeeting(result.meeting.id, result.meeting.password);
  ...
  async function joinMeeting(joinId = meetingId, joinPassword = password) {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.error ?? "Unable to join meeting.");
      setConnection(result); setMode(null);
    } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to join meeting."); }
    finally { setLoading(false); }
  }

  if (connection) return <LiveMeeting connection={connection} onLeave={() => setConnection(null)} />;
