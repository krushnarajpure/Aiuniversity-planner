...
export function AptitudeTestSession({ sessionId }: { sessionId: string }) {
    ...
    async function saveAnswer() {
        ...
        try {
            ...
            const payload = await response.json().catch(() => ({}));
            if (!response.ok) throw new Error(payload?.error || "Unable to save this answer. Please try again.");
            return true;
        } finally {
            setSaving(false);
        }
    ...
    const handleNextQuestion = async () => {
        if (!sessionData || saving || submitting) return;
            ...
            <div className="card">
                ...
                <div className="flex gap-2 flex-wrap pt-6 border-t border-slate-200 dark:border-slate-700">
                    <button
                        ...
                        disabled={!selectedAnswer || saving || submitting}
            ...
            <div className="flex gap-4">
                <button
                    onClick={handlePreviousQuestion}
                        disabled={sessionData.currentQuestionNo === 1 || saving || submitting}
