...
function MeetingWorkspace({ meetingId, onLeave }: { meetingId: string; onLeave: () => void }) {
  ...
  const remoteParticipants = participants.filter((participant) => !(participant instanceof LocalParticipant));
  const solo = participants.length <= 1;
  const gridParticipants = solo ? participants : remoteParticipants;
...
function MeetingControls({ localParticipant, drawer, setDrawer, onLeave }: { localParticipant: LocalParticipant; drawer: Drawer; setDrawer: (value: Drawer) => void; onLeave: () => void }) {
