...
export function ForgotPasswordForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try { const response = await fetch("/api/auth/forgot-password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email }) }); const data = await response.json(); toast.success(data.message || "If an account exists for that email, a password reset link has been sent."); } catch { toast.error("Could not request a password reset. Please try again."); } finally { setPending(false); }
  }
